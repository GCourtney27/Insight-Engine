...
bool Sprite::Initialize(ID3D11Device * device, ID3D11DeviceContext * deviceContext, float width, float height, std::string spritePath, ConstantBuffer<CB_VS_vertexshader_2d> & CB_VS_PerObject_2d)
{
	...
	HRESULT hr = verticies.Initialize(device, vertexData.data(), (UINT)vertexData.size());
	COM_ERROR_IF_FAILED(hr, "Failed to initialize vertex buffer for sprite.");

	hr = indicies.Initialize(device, indexData.data(), (UINT)indexData.size());
...
void Sprite::Draw(XMMATRIX orthoMatrix)
{
	...
	deviceContext->IASetVertexBuffers(0, 1, verticies.GetAddressOf(), verticies.StridePtr(), &offsets);
	deviceContext->IASetIndexBuffer(indicies.Get(), DXGI_FORMAT::DXGI_FORMAT_R32_UINT, 0);
	deviceContext->DrawIndexed(indicies.IndexCount(), 0, 0);
}
