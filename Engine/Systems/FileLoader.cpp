...
bool FileLoader::LoadSceneFromFile(const std::string & sceneLocation, Scene * scene, ID3D11Device * device, ID3D11DeviceContext * deviceContext, ConstantBuffer<CB_VS_vertexshader>& cb_vs_vertexshader)
{
	...
	while (!f.eof())
	{
		...
		if (line[0] == 's')
		{
			...
			{
				if (!entity->GetComponent<MeshRenderer>()->Initialize(modelFilepath, device, deviceContext, cb_vs_vertexshader))
					return false;
			}*/
			entity->SetTagID(name);
...
bool FileLoader::LoadSceneFromFileGO(const std::string & sceneLocation, std::vector<RenderableGameObject*>& gameObjectsToPopulate, ID3D11Device * device, ID3D11DeviceContext * deviceContext, ConstantBuffer<CB_VS_vertexshader>& cb_vs_vertexshader)
{
	std::fstream f(sceneLocation);
	if (!f.is_open())
		return false;
