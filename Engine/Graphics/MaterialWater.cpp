...
bool MaterialWater::Initiailze(ID3D11Device * device, ID3D11DeviceContext * deviceContext, const rapidjson::Value& assetsInformation, eFlags materialAttributeFlags)
{
...
bool MaterialWater::Initiailze(ID3D11Device * device, ID3D11DeviceContext * deviceContext, eFlags materialAttributeFlags)
{
	this->m_pDevice = device;
	this->m_pDeviceContext = deviceContext;
	this->m_flags = materialAttributeFlags;
...
void MaterialWater::WriteToJSON(rapidjson::PrettyWriter<rapidjson::StringBuffer>& writer)
{
...
bool MaterialWater::InitializeJOSNPiplineAssets(const rapidjson::Value& assetsInformation)
{
...
bool MaterialWater::InitializePiplineAssets()
{
...
void MaterialWater::InitializeShaders()
{
	...
	if (!m_vertexShader.Initialize(m_pDevice, m_shaderFolder + L"PBR_Water_vs.cso", defaultLayout3D, numElements3D))
		ErrorLogger::Log("Failed to initialize default vertex PBR shader");

	if (!m_pixelShader.Initialize(m_pDevice, m_shaderFolder + L"PBR_Water_ps.cso"))
...
void MaterialWater::PSSetShaderResources()
{
