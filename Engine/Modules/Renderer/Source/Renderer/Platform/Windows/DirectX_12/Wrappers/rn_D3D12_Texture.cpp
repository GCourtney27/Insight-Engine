...
namespace Retina {
	...
	ieD3D12Texture::ieD3D12Texture(RN_TEXTURE_INFO CreateInfo, CDescriptorHeapWrapper& srvHeapHandle)
		: Texture(CreateInfo),
	...
	bool ieD3D12Texture::Init(RN_TEXTURE_INFO createInfo, CDescriptorHeapWrapper& srvHeapHandle)
	{
	...
	void ieD3D12Texture::InitDDSTexture(CDescriptorHeapWrapper& srvHeapHandle)
	{
		...
		if (FAILED(hr)) {
			RN_CORE_ERROR("Failed to load DDS texture from file with path \"{0}\"", StringHelper::WideToString(m_TextureInfo.Filepath));
		}
	...
	bool ieD3D12Texture::InitTextureFromFile(CDescriptorHeapWrapper& srvHeapHandle)
	{
		...
		if (FAILED(hr)) {
			RN_CORE_ERROR("Failed to Create WIC texture from file with path \"{0}\"", StringHelper::WideToString(m_TextureInfo.Filepath));
		}
	...
	UINT ieD3D12Texture::GetRootParameterIndexForTextureType(eTextureType TextureType)
	{
		...
		default:
		{
			RN_CORE_WARN("Failed to get root parameter index with texture type: {0}", TextureType);
			break;
		}
		}
		return 6;
