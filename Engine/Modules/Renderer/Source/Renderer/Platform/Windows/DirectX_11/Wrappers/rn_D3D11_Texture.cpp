...
namespace Retina {
	...
	ieD3D11Texture::ieD3D11Texture(RN_TEXTURE_INFO CreateInfo)
		: Texture(CreateInfo)
	{
		Init();
	}
	...
	uint32_t ieD3D11Texture::GetShaderRegisterLocation()
	{
		...
		default:
		{
			RN_CORE_ERROR("Failed to determine shader register of texture with type \"{0}\" in D3D 11 context.", m_TextureInfo.Type);
		}
		}
		return - 1;
	}
