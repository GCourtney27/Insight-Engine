...
namespace Retina {
	...
	{
		RN_CORE_WARN("Mesh being moved in memory.");
	...
	void Mesh::CreateBuffers(Verticies& Verticies, Indices& Indices)
	{
		...
		{
			RN_CORE_FATAL(L"Mesh trying to be created before the renderer has been initialized.");
			break;
		}
		default:
		{
			RN_CORE_ERROR("Failed to determine vertex buffer type for mesh.");
			break;
		}
		}
	}
