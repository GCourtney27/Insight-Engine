...
namespace Retina {
	...
	bool TextureManager::LoadResourcesFromJson(const rapidjson::Value& JsonTextures)
	{
		...
		for (rapidjson::SizeType i = 0; i < JsonTextures.Size(); i++) {
			...
			Texture::RN_TEXTURE_INFO TexInfo = {};
			TexInfo.Id = ID;
	...
	StrongTexturePtr TextureManager::GetTextureByID(Texture::ID TextureID, Texture::eTextureType TextureType)
	{
		...
		default:
		{
			RN_CORE_WARN("Failed to get texture handle for texture with ID: {0}", TextureID);
			break;
		}
		}
	...
	bool TextureManager::LoadDefaultTextures()
	{
		...
		// Albedo
		RN_TEXTURE_INFO AlbedoTexInfo = {};
		size_t TextureInfo = sizeof(RN_TEXTURE_INFO);
		...
		// Metallic
		RN_TEXTURE_INFO MetallicTexInfo = {};
		...
		// Roughness
		RN_TEXTURE_INFO RoughnessTexInfo = {};
		...
		default:
		{
			RN_CORE_ERROR("Failed to load default textures for api: {0}", Renderer::GetAPI());
			break;
		}
		}
		return true;
	...
	void TextureManager::RegisterTextureByType(const RN_TEXTURE_INFO TexInfo)
	{
		switch (Renderer::GetAPI())
		{
			case Renderer::eTargetRenderAPI::D3D_11:
			{
				...
				default:
				{
					RN_CORE_WARN("Failed to identify texture to create with ID of {0}", TexInfo.Id);
					break;
				}
				}
				break;
			...
			default:
			{
				RN_CORE_ERROR("Failed to determine graphics api to initialize texture. The renderer may not have been initialized yet.");
				break;
			}
