...
#include "Retina/Input/Windows_Input.h"
...
namespace Retina {
	...
	bool MonoScriptManager::Init()
	{
		if (!m_ManagerIsInitialized) {
			...
			if (!m_pDomain) {
				RN_CORE_ERROR("Failed to initialize mono domain.");
				return false;
			}
		...
		if (!m_pAssembly) {
			RN_CORE_ERROR("Failed to open mono assembly with path: {0}", m_AssemblyDir);
			return false;
		}

		m_pImage = mono_assembly_get_image(m_pAssembly);
		if (!m_pImage) {
			RN_CORE_ERROR("Failed to get image from mono assembly.");
			return false;
		}
