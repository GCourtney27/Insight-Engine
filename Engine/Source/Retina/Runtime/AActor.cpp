...
#include "Retina/Runtime/Components/CSharp_Scirpt_Component.h"
#include "Retina/Runtime/Components/Sphere_Collider.h"
	...
	namespace Runtime {
		...
		void AActor::RenderSceneHeirarchy()
		{
			...
			if (ImGui::IsItemClicked()) {
				RN_STRIP_FOR_GAME_DIST(Application::Get().GetEditorLayer().SetSelectedActor(this);)
			}
		...
		void AActor::OnImGuiRender()
		{
			...
			// Add new component drop down
			{
				...
				if (ImGui::Combo("Add Component", &currentIndex, availableComponents, IM_ARRAYSIZE(availableComponents))) {
					...
					case 1:
					{
						RN_CORE_INFO("Adding Static Mesh Component to \"{0}\"", AActor::GetDisplayName());
						StaticMeshComponent* ptr = AActor::CreateDefaultSubobject<StaticMeshComponent>();
					...
					case 2:
					{
						RN_CORE_INFO("Adding C-Sharp Script Component to \"{0}\"", AActor::GetDisplayName());
						CSharpScriptComponent* ptr = AActor::CreateDefaultSubobject<CSharpScriptComponent>();
						break;
					}
					default:
					{
						RN_CORE_INFO("Failed to determine component to add to actor \"{0}\" with index of \"{1}\"", AActor::GetDisplayName(), currentIndex);
						break;
					}
					}
				}
		...
		{
			RN_CORE_ASSERT(std::find(m_Components.begin(), m_Components.end(), component) != m_Components.end(), "Could not find Component in Actor list while attempting to delete");
		...
		{
			RN_CORE_INFO("Physics Collision");
			return false;
		}
