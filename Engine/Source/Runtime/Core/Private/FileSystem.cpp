...
namespace Insight {
	...
	bool FileSystem::LoadSceneFromJson(const std::string& FileName, Scene* pScene)
	{
		...
		// Load in Actors.json last once resources have been intialized
		{
			...
			for (rapidjson::SizeType a = 0; a < SceneObjects.Size(); a++)
			{
				...
				FString ActorDisplayName = StringHelper::StringToWide(DisplayName);
				FString ActorType = StringHelper::StringToWide(Type);
	...
	FString FileSystem::GetShaderPathW(const TChar* Shader)
	{
		FString Path = L"Shaders/";
		Path.append(Shader);
		return Path;
	}
}
