...
namespace Insight {
	...
	void RayTraceHelpers::CreateRTPipeline()
	{
		...
		ThrowIfFailed(RayGenShader.LoadFromFile(FileSystem::GetShaderPathW(L"RayGen.lib.cso").c_str()), TEXT("Failed to load Ray Gen library from cso."));
		// Miss
		D3D12Shader MissShader;
		ThrowIfFailed(MissShader.LoadFromFile(FileSystem::GetShaderPathW(L"Miss.lib.cso").c_str()), TEXT("Failed to load Miss library from cso."));
		// Hit
		D3D12Shader ClosestShader;
		ThrowIfFailed(ClosestShader.LoadFromFile(FileSystem::GetShaderPathW(L"ClosestHit.lib.cso").c_str()), TEXT("Failed to load Closest Hit library from cso."));
		// Shadow
		D3D12Shader ShadowShader;
		ThrowIfFailed(ShadowShader.LoadFromFile(FileSystem::GetShaderPathW(L"ShadowRay.lib.cso").c_str()), TEXT("Failed to load Shadow Ray library from cso."));
