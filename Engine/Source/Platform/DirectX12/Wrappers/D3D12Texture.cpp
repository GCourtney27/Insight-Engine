...
namespace Insight {
	...
	void ieD3D12Texture::InitDDSTexture(CDescriptorHeapWrapper& srvHeapHandle)
	{
		...
		if (FAILED(hr)) {
			IE_LOG(Error, "Failed to load DDS texture from file with path \"{0}\"", StringHelper::WideToString(m_TextureInfo.Filepath));
		}

		m_D3DTextureDesc = m_pTexture->GetDesc();
		if (!ResourceUpload.IsSupportedForGenerateMips(m_D3DTextureDesc.Format)) {
			//IE_LOG(Warning, "Mip map generation not supported for texture: {0}", m_DisplayName);
		}
	...
	bool ieD3D12Texture::InitTextureFromFile(CDescriptorHeapWrapper& srvHeapHandle)
	{
		...
		if (FAILED(hr)) {
			IE_LOG(Error, "Failed to Create WIC texture from file with path \"{0}\"", StringHelper::WideToString(m_TextureInfo.Filepath));
			return false;
		}
	...
	UINT ieD3D12Texture::GetRootParameterIndexForTextureType(eTextureType TextureType)
	{
		...
		default:
		{
			IE_LOG(Warning, "Failed to get root parameter index with texture type: {0}", TextureType);
			break;
		}
		}
		return 6;
