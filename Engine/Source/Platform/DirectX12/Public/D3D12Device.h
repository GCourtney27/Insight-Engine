	...
	namespace Graphics
	{
		namespace DX12
		{
			...
			public:
				...
				virtual void CreateDescriptorHeap(const FString&& DebugHeapName, EResourceHeapType&& Type, UInt32&& MaxCount, IDescriptorHeap** ppOutHeap) override;
				virtual void CreateColorBuffer(const FString& Name, UInt32 Width, UInt32 Height, UInt32 NumMips, EFormat Format, IColorBuffer** ppOutColorBuffer) override;
				virtual void CreateDepthBuffer(const FString& Name, UInt32 Width, UInt32 Height, EFormat Format, IDepthBuffer** ppOutDepthBuffer) override;
