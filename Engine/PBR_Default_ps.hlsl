...
float4 main(PS_INPUT input) : SV_TARGET
{
   ...
   //float3 N = input.inNormal;
	...
	float3 radiance = albedo * diffuseLight; // Works
	...
	float3 diffuse = irradianceMapSRV.Sample(samplerState, N).rgb * albedo * kD_IBL;
	// Specular IBL
	const float MAX_REFLECTION_LOD = 4.0f;
	float3 prefilteredColor = prefilterMapSRV.SampleLevel(samplerState, reflect(-V, N), roughness * MAX_REFLECTION_LOD).rgb;
