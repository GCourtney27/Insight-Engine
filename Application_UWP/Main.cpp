...
public:
    ...
    void Initialize(CoreApplicationView const & applicationView)
    {
        ...
        m_App = Insight::CreateApplication();
    }
    ...
    void SetWindow(CoreWindow const & window)
    {
        ...
        m_Window = std::make_shared<Insight::UWPWindow>(Insight::WindowProps());
        if (m_Window->Init(static_cast<::IUnknown*>(winrt::get_abi(window))))
        {
            IE_FATAL_ERROR(L"Fatal Error: Failed to initialize window.");
            //return false;
        }
        m_Window->SetEventCallback(std::bind(&Insight::Application::OnEvent, m_App, std::placeholders::_1));
...
protected:
    ...
    void OnActivated(CoreApplicationView const & /*applicationView*/, IActivatedEventArgs const & args)
    {
        ...
        auto desiredSize = winrt::Windows::Foundation::Size(ConvertPixelsToDips(w), ConvertPixelsToDips(h));

        ApplicationView::PreferredLaunchViewSize(desiredSize);

        auto view = ApplicationView::GetForCurrentView();

        auto minSize = winrt::Windows::Foundation::Size(ConvertPixelsToDips(320), ConvertPixelsToDips(200));
...
private:
    ...
    Insight::Application*   m_App;
    std::shared_ptr<Insight::UWPWindow> m_Window;
